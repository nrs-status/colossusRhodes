rec {
  ...
  typecheckFields = import ../typechecklib/typecheckFields.nix { inherit libs; };
  typecheckPredicates = import ../typechecklib/typecheckFields.nix;
  targetWithDebug = import (x + /resources/plugins/cmp.nix) {inherit libs; withDebug = true; };
  mapping = builtins.mapAttrs (field: val: val.function targetWithDebug.typecheckarg.target) predicatesOfType;
  filtering = libs.pkgslib.filterAttrs (field: val: val == false) mapping;
  type = importedTypesSource.types.PluginSlice;
  typeName = "PluginSlice";
  target = targetWithDebug.typecheckarg;
  typecheck_1 = typecheckFields {
    inherit type;
